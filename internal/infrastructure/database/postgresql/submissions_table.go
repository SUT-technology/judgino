...
func (c submissionsTable) GetSubmissionsByFilter(ctx context.Context, userId uint, questionId uint, submissionFilter string, finalFilter string, pageParam uint) ([]*entity.Submission, error) {
	var submissions []*entity.Submission
	var query *gorm.DB
	if submissionFilter == "mine" && userId != 0 {
		query = c.db.Where("user_id = ?", userId).Order("submit_time").Offset(10 * (int(pageParam) - 1) - 1).Limit(10)
	} else {
		query = c.db.Order("submit_time").Offset(10 * (int(pageParam) - 1) - 1).Limit(10)
	}
